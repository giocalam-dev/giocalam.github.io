...
const SCORING = {
  ...
  billiardino:  { win: 3, winAdv: 2, lossAdv: 1, loss: 0 }
};

function hasAdvantageScoring(sport) {
  return sport === 'pallavolo' || sport === 'billiardino';
}
...
function updateKnockoutBrackets(sport, data) {
  ...
  if (sport === 'calcio') {
    ...
    updateBasketBracket(matches, data);
  }
...
function updateBasketBracket(matches, data) {
  const groupA = matches.filter(m => m.group === 'A' && m.phase === 'group');
  const groupB = matches.filter(m => m.group === 'B' && m.phase === 'group');
  const allGroupsDone = groupA.length > 0 && groupB.length > 0
    && groupA.every(m => m.played) && groupB.every(m => m.played);
  ...
  if (allGroupsDone) {
    const stA = getGroupStandings(matches, 'basket', 'A', data.groups ? data.groups.A : null);
    const stB = getGroupStandings(matches, 'basket', 'B', data.groups ? data.groups.B : null);
